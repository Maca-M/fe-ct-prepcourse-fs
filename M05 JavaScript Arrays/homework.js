...
function numeroMasGrande(arrayOfNums) {
   ...
   let nummay = 0;
   for (let i = 0; i < arrayOfNums.length; i++) {
      if (arrayOfNums[i]>nummay) nummay = arrayOfNums[i];
   }
   return nummay;
}
...
function empiezaConNueve(num) {
   ...
   // Tu código:
   let numts = String(num);
   let array = numts.split("");
...
function mayorACien(array) {
   ...
   let ent = [];
   for (let i = 0; i < array.length; i++){
      if (array[i]>100) ent.push(array[i]);
   }
   return ent;
}
